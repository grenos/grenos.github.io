class UI {
  ...
  printMovies(topMoviesRes) {
    ...
    const filteredMovies = topMovies.reduce(
      (acc, movie) => {
  ...
  printSeries(topSeriesRes) {
    ...
    const filteredSeries = topSeries.reduce(
      (acc, serie) => {
  ...
  printSearchCat(searchCatalogueRes) {
    ...
    const searchQueryMovie =
      searchCatalogueRes.searchMovieCatalogueInfo.results;
    const filteredMovies = searchQueryMovie.reduce(
      (acc, movie) => {
        if (movie.backdrop_path == null) {
          movie.rejectionReason = 'no backdrop';
          acc.rejects.push(movie);
    ...
    const searchQuerySerie =
      searchCatalogueRes.searchSeriesCatalogueInfo.results;
    const filteredSeries = searchQuerySerie.reduce(
      (acc, serie) => {
        if (serie.backdrop_path == null) {
          serie.rejectionReason = 'no backdrop';
          acc.rejects.push(serie);
  ...
  printModalMovie(searchMovieIdRes) {
    ...
    cast.forEach(cast => {
      actors += `
                    <a href="#!" class="actor" data-actor="${
                      cast.id
                    }" onclick="printActor(event)">${cast.name}</a>
                `;
    });
                        ...
                        ${
                          searchMovieIdRes.movieDetailsInfo.videos.results < 1
                            ...
                            <div class="col-md-12 mx-auto video-container">
                                ...
                                }?enablejsapi=1&fs=0&iv_load_policy=3&rel=0&showinfo=0" id="trailer-player" frameborder="0" allowfullscreen class="video"></iframe> 
                            </div> `
                        } 
                            
                            ${
                              searchMovieIdRes.movieDetailsInfo.similar
                                .results < 1
                                ? ''
                                : `
    ...
    output = `
            ...
            <a href="${site}" target="_blank"><i class="fas fa-globe"></i></a>
                ...
                <ul class="list-group">
                    ...
                    <li class="list-group-item ml-5"><p>Network: <span>${
                      searchSerieIdRes.networks[0].name
                    }</span></p></li>
                    <li class="list-group-item ml-5"><p>Language: <span>${lang}</span></p></li>
                    <li class="list-group-item ml-5"><p>Series Started: <span>${firstD}</span> Latest Air Date: <span>${lastD}</span></p></li>
                    <li class="list-group-item ml-5"><p>Runtime: ~ <span>${
                      searchSerieIdRes.episode_run_time[0]
                    }</span> minutes</p></li>
                    <li class="list-group-item ml-5"><p>Popularity: <span><i class="fas fa-heart fa-sm"></i> ${votes}</span> votes (${pop})</p></li>
                </ul>
                </div>
                ...
                ${
                  searchSerieIdRes.videos.results < 1
                    ...
                    <div class="col-md-12 mx-auto video-container">
                        <iframe src="https://www.youtube.com/embed/${
                          searchSerieIdRes.videos.results[0].key
                        }?enablejsapi=1&fs=0&iv_load_policy=3&rel=0&showinfo=0" id="trailer-player" frameborder="0" allowfullscreen class="video"></iframe> 
                    </div> `
                }  
      ...
      poster += `
                <div class="slick-item">
                    <img src="https://image.tmdb.org/t/p/w154/${
                      movie.poster_path
                    }"  id="${movie.id}" data-id="movie" alt="Similar">
                </div>
            `;
    });
    ...
    searchSerieIdRes.similar.results.forEach(serie => {
      poster += `
                <div class="slick-item">
                    <img src="https://image.tmdb.org/t/p/w154/${
                      serie.poster_path
                    }"  id="${serie.id}" data-id="serie" alt="Similar">
                </div>
            `;
    });
  ...
  openSimilar() {
    ...
    document
      ...
      .classList.toggle('car-cont-active');
  }
  ...
  printMovieByGenre(movieGenreRes) {
    ...
    const filteredMovies = movieGenre.reduce(
      (acc, movie) => {
        if (movie.poster_path == null) {
          movie.rejectionReason = 'no photo';
          acc.rejects.push(movie);
  ...
  printSeriesByGenres(serieGenreRes) {
    ...
    const filteredSeries = seriesGenre.reduce(
      (acc, serie) => {
        if (serie.poster_path == null) {
          serie.rejectionReason = 'no photo';
          acc.rejects.push(serie);
  ...
  printActor(getActorRes) {
    ...
    const filteredCredits = getActorRes.combined_credits.cast.reduce(
      (acc, credit) => {
    ...
    socialLinks.forEach(link => {
      ...
      socials += `
                ${
                  fb
                    ? `<a target="_blank" href="https://www.facebook.com/${fb}"><i class="fab fa-facebook-square"></i></a>`
                    : ''
                }
                ${
                  insta
                    ? `<a target="_blank" href="https://www.instagram.com/${insta}"><i class="fab fa-instagram"></i></a>`
                    : ''
                }
                ${
                  twit
                    ? `<a target="_blank" href="https://twitter.com/${twit}"> <i class="fab fa-twitter-square"></i></a>`
                    : ''
                }
                ${
                  imdb
                    ? `<a target="_blank" href="https://www.imdb.com/name/${imdb}"><i class="fab fa-imdb"></i></a>`
                    : ''
                }
            `;
    });
